...
for i in {1..3}
do
    iteration_name="LLaMA3_iter${i}_DPO"
    jsonl_input="RLHFlow/ultrafeedback_iter${i}"
    json_output="${base_path}/${iteration_prefix}${i}_${iteration_name}_DPO"
    model_output="${base_path}/${iteration_prefix}${i}_${iteration_name}_DPO_reward.json"
    ...
    if [ $i -eq 1 ]; then
        ...
        model_path="LLaMA3_iter${previous_iteration}_DPO"
        sam_model_path=$initial_model
    else
        previous_iteration=$((i-1))
        sam_previous_iteration=$((i-2))
        model_path="LLaMA3_iter${previous_iteration}_DPO"
        sam_model_path="LLaMA3_iter${sam_previous_iteration}_DPO"
    fi
